# Script to plot line ratio maps of H2/Bry and FeII/Bry but only using
# pixels where SiVI is detected.
...
save_dir = proj_dir + 'line_ratio_maps/sivi_detected/'
...
for name in sample.index:
	...
	if sample.loc[name, 'type'] == 'A':
		...
		mask_total = h2_mask & bry_mask & feii_mask & sivi_mask

		# Calculate the H2/Bry and FeII/Bry line ratios
		# Mask out the pixels where SiVI isn't detected.
		line1 = h2_flux/bry_flux
		line1[~sivi_mask] = np.nan

		line2 = feii_flux/bry_flux
		line2[~sivi_mask] = np.nan
		...
		fig.savefig(save_dir+name+'_line_ratio_maps_sivi_detected.pdf', bbox_inches='tight')
		fig2.savefig(save_dir+name+'_diagnostic_regions_sivi_detected.pdf', bbox_inches='tight')
		plt.close(fig)
		plt.close(fig2)
